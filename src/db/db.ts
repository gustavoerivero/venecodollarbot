...
export const getByColumn = async (
  ...
  try {
    ...
    const whereClause = columnsArray.map((column, index) => `${column} = ${dataArray[index]}`).join(' AND ');
...
export const update = async (
  ...
  try {
    ...
    const setClause = columnsArray.map((column, index) => `${column} = "${dataArray[index]}"`).join(', ');
